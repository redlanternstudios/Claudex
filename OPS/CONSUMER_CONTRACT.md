...
3. Read `bridge_file`.
4. Read `qa_protocol_file` and `questions_index_file` when the product needs a cross-system answer.
5. Never create a product local bridge or local question queue as fallback state.
...
Cross-system questions and answers are written only to `OPS/questions/` in Claudex. Answers must follow `OPS/QA_PROTOCOL.md`.
...
Missing `OPS/QA_PROTOCOL.md` or `OPS/questions/INDEX.md`: `SYNC YELLOW` for cross-system question work, not for ordinary product code work.
